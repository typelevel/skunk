  ...
  test("error in alloc is rethrown to caller (deferral completion following errored cleanup)") {
    resourceYielding(IO(1), IO.raiseError(AllocFailure())).flatMap { r =>
      ...
      p.use { r =>
        for {
          d  <- Deferred[IO, Unit]
          f1 <- r(natchez.Trace[IO]).use(n => assertEqual("n should be 1", n, 1) *> d.get).assertFailsWith[ResetFailure].start
          f2 <- r(natchez.Trace[IO]).use(_ => fail[Int]("should never get here")).assertFailsWith[AllocFailure].start
          _  <- d.complete(())
          _  <- f1.join
          _  <- f2.join
        } yield ()
