...
class StringOpsMacros(val c: whitebox.Context) {
  ...
  def sql_impl(argSeq: Tree*): Tree = {
    ...
    val (finalParts, encoders) : (List[Tree /* part */], List[Tree] /* encoder */) =
      (parts zip args).foldRight((List(q"skunk.syntax.StringOps.Str(${parts.last})"), List.empty[Tree])) {
        ...
        case ((part @ Literal(Constant(str: String)), arg), (tail, es)) =>
          ...
          if (str.endsWith("#")) {
            ...
            if (argType <:< StringType) {
              val p1 = q"skunk.syntax.StringOps.Str(${str.init}.concat($arg))"
              (p1 :: tail, es)
            } else
              ...
              val p1 = q"skunk.syntax.StringOps.Str($part)"
              val p2 = q"skunk.syntax.StringOps.Par($arg.oids.length)"
              (p1 :: p2 :: tail, arg :: es)

          } else if (argType <:< VoidFragmentType) {

              val p1 = q"skunk.syntax.StringOps.Str($part)"
              val p2 = q"skunk.syntax.StringOps.Emb($arg.parts)"
              (p1 :: p2 :: tail, es)
