...
object implicits {
  ...
  private def liftT[F[_]: MonadCancel[*[_], Throwable]](
    ...
    Kleisli { req =>
      ...
      val spanR  = entryPoint.continueOrElseRoot(req.uri.path.toString, kernel)
      OptionT {
        spanR.use { span =>
   ...
   */
  def natchezMiddleware[F[_]: MonadCancel[*[_], Throwable]: Trace](routes: HttpRoutes[F]): HttpRoutes[F] =
    Kleisli { req =>
