...
object Http4sExample extends IOApp {
   ...
   * `Countries` resource.
   */
  def countriesFromSocketGroup[F[_]: Async: Trace: Console](
  ...
  def pool[F[_]: Async : Console: Trace : Network]: Resource[F, Resource[F, Countries[F]]] =
    for {
      r  <- Pool.of(countriesFromSocketGroup(Network[F]), 10)(pooledCountriesRecycler)
   ...
   */
  def routes[F[_]: Async : Console: Trace : Network]: Resource[F, HttpRoutes[F]] =
  ...
  def runR[F[_]: Async : Console]: Resource[F, Unit] =
    for {
      ep <- entryPoint[F]
